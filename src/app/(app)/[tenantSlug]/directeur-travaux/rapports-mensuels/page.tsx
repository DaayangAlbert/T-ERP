...
function fmtFCFA(n: string) {
  const v = Number(n);
  if (!Number.isFinite(v)) return "—";
  return new Intl.NumberFormat("fr-FR").format(Math.round(v)) + " FCFA";
}

export default function DtravMonthlyReportsPage() {
  const router = useRouter();
  const params = useParams<{ tenantSlug: string }>();
  const [filter, setFilter] = useState<DtravReportStatus | "">("");
  const [createOpen, setCreateOpen] = useState(false);
  const { data, isLoading } = useDtravReports({ status: filter || undefined });
  const create = useCreateDtravReport();
  const del = useDeleteDtravReport();
  ...
  const handleCreate = (period: string) => {
    create.mutate(
      ...
      { onSuccess: (res) => router.push(`/${params.tenantSlug}/directeur-travaux/rapports-mensuels/${res.id}`) },
    );
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-line bg-white p-3 shadow-card">
        <select value={filter} onChange={(e) => setFilter(e.target.value as DtravReportStatus | "")} className="h-9 rounded-md border border-line bg-white px-2 text-[12.5px]">
          ...
          <table className="w-full min-w-[800px] text-[12.5px]">
            <thead className="bg-surface-alt text-[11px] uppercase tracking-wide text-ink-3">
              <tr>
                ...
                <th className="py-2 text-right">CA produit</th>
                <th className="py-2 text-right">Marge</th>
                <th className="py-2 text-right">Encours</th>
                <th className="py-2 text-right">CDT</th>
                  ...
                  <td className="py-2.5 pl-3">
                    ...
                    {r.submittedAt && (
                      <div className="mt-0.5 inline-flex items-center gap-1 text-[10.5px] text-ink-3">
                        <Clock className="h-2.5 w-2.5" /> {new Date(r.submittedAt).toLocaleDateString("fr-FR")}
                      </div>
                    )}
                  </td>
                  <td className="py-2.5">
                    ...
                    {r.status === "VALIDATED" && r.validatedBy && (
                      <div className="mt-0.5 flex items-center gap-1 text-[10px] text-emerald-700">
                        <CheckCircle2 className="h-2.5 w-2.5" /> {r.validatedBy}
                      </div>
                    )}
                  </td>
                  <td className="py-2.5 text-right font-mono tabular-nums">{fmtFCFA(r.revenueProducedXAF)}</td>
                  <td className="py-2.5 text-right font-mono tabular-nums">{r.marginPercent.toFixed(1)} %</td>
                  <td className="py-2.5 text-right font-mono tabular-nums">{fmtFCFA(r.receivablesXAF)}</td>
                  <td className="py-2.5 text-right font-mono tabular-nums">{r.cdtCount}</td>
                  <td className="py-2.5 pr-3 text-right">
                    <div className="inline-flex items-center gap-1">
                      <button onClick={() => router.push(`/${params.tenantSlug}/directeur-travaux/rapports-mensuels/${r.id}`)} className="grid h-7 w-7 place-items-center rounded text-ink-3 hover:bg-white" title="Ouvrir">
                        <Pencil className="h-3.5 w-3.5" />
                      </button>
                      {(r.status === "SUBMITTED" || r.status === "VALIDATED") && (
                        <a href={`/api/dtrav/monthly-reports/${r.id}/pdf`} target="_blank" rel="noopener" className="grid h-7 w-7 place-items-center rounded text-violet-700 hover:bg-violet-50" title="PDF">
