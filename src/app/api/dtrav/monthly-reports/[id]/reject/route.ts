...
import { rejectDtravMonthlyReportSchema } from "@/schemas/dtrav-monthly-report";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const body = rejectDtravMonthlyReportSchema.parse(await req.json());
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.notification.create({
        data: {
          ...
          title: "Votre rapport mensuel DTrav est refusé",
          body: body.reason,
          link: `/directeur-travaux/rapports-mensuels/${id}`,
        },
      });
    });
    ...
    console.error("[POST /api/dtrav/monthly-reports/:id/reject]", err);
