...
export async function POST(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.qhseMonthlyReport.update({
      where: { id },
      data: { status: "VALIDATED", validatedById: session.sub, validatedAt: new Date(), rejectionReason: null },
    });
    await tx.notification.create({
      data: {
        userId: report.authorId,
        type: "report_validated",
        title: "Votre rapport mensuel QHSE est validé",
        body: "Le DG a validé votre rapport.",
