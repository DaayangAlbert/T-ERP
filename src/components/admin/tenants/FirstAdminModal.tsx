...
export function FirstAdminModal({ tenantId, tenantName, onClose }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    setBusy(true);
    const res = await fetch(`/api/admin/tenants/${tenantId}/first-admin`, {
      method: "POST",
    ...
    if (!res.ok) {
      setError(
        json.error ??
          (res.status === 409
            ? "Un admin existe déjà pour ce tenant"
            : "Erreur création"),
      );
      return;
    }
      ...
      >
        ...
        >
          <h3 className="text-sm font-semibold text-white">
            Créer le 1<sup>er</sup> admin · {tenantName}
          </h3>
          <button
            onClick={onClose}
            aria-label="Fermer"
        ...
        {success ? (
          <div className="space-y-4 px-5 py-5">
            <div className="rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-xs text-emerald-200">
              ✓ Compte admin créé pour <strong>{success.user.firstName} {success.user.lastName}</strong>.
              <br />
              {success.message}
            </div>
            <div
            ...
            <p className="text-[11px] text-amber-200/80">
              ⚠ Communique ces identifiants au client par WhatsApp / SMS / téléphone — pas par
          ...
          <form onSubmit={submit} className="space-y-3 px-5 py-5">
            <Field label="Email pro" required>
              <input
                ...
                placeholder="admin@aa-hatlad.cm"
            ...
            <p className="rounded-md bg-cyan-500/10 px-3 py-2 text-[11px] text-cyan-200">
              Un mot de passe initial de 17 caractères sera généré automatiquement. Tu le verras
              après création pour le transmettre au client.
            </p>
